  ...
  <BaseWindow className="setting" :item="item">
    <template v-slot:body>
      ...
      <div class="setting-container">
        ...
        <div :style="[navIndex==3?'':'display:none']" class="padding-10 page-url-config">
          ...
          <ul>
            <template v-for="item in openUrlState.list" :key="item">
              <li class="margin-10px background-white-translucent padding-10 border-5-radius flex">
                <div class="flex-1 font-size-10">{{item}}</div>
        ...
        <div :style="[navIndex==4?'':'display:none']" class="page-about padding-10">
          CoolDesktop是一款服务器管理软件，提供强大的软件扩展能力，任何开发者都将可以自行开发管理软件并挂在到本系统上。
          <br>
          <br>
          详细请查看官方文档
          <br>
...
const resetPassClick = () => {
  ...
  systemApi.apiResetLogoPasswd().then((e) => {
    loadingWindow.closeWindow()
    coolWindow.startNewSuccessMessageDialog(e.data)
  })
}
...
const removeOpenUrl = (url) => {
  systemApi.apiRemoveOpenUrl(url).then((res) => {
...
const showAllOpenUrl = () => {
  systemApi.apiGetOpenUrl().then((res) => {
    openUrlState.list = res.data.data
  })
}
showCurrentSystemWallpaper()
showAllOpenUrl()
